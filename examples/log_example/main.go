...
func doSomething(client *mcp_golang.Client) {
...
func main() {
	...
	client := mcp_golang.NewClient(clientTransport, mcp_golang.WithNotificationHandler("notifications/message", func(notification *transport.BaseJSONRPCNotification) error {
		var params struct {
	...
	for _, level := range []mcp_golang.Level{
		mcp_golang.LevelDebug,
		mcp_golang.LevelInfo,
		mcp_golang.LevelNotice,
		mcp_golang.LevelWarning,
		mcp_golang.LevelError,
	} {
		log.Println("\n\n\n")
