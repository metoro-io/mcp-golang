...
// TestMessageDeserializationV2 tests the parsing of different JSON-RPC message types.
// Proper message type detection and parsing is critical for protocol operation.
// It tests:
// 1. Request messages (with method and ID)
// 2. Notification messages (with method, no ID)
...
// 4. Success responses (with result)
// Each message type must be correctly identified and parsed to maintain protocol integrity.
func TestMessageDeserializationV2(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		wantType transport.BaseMessageType
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			msg, err := deserializeMessageV2(tt.input)
			if err != nil {
				t.Errorf("deserializeMessage failed: %v", err)
			}
			if msg == nil {
	...
	t.Run("request", func(t *testing.T) {
		msg, err := deserializeMessageV2(`{"jsonrpc":"2.0","id":1,"method":"test","params":{}}`)
		assert.NoError(t, err)
		assert.Equal(t, transport.BaseMessageTypeJSONRPCRequestType, msg.Type)
		assert.Equal(t, "2.0", msg.JsonRpcRequest.Jsonrpc)
		assert.Equal(t, "test", msg.JsonRpcRequest.Method)
	...
	t.Run("notification", func(t *testing.T) {
		msg, err := deserializeMessageV2(`{"jsonrpc":"2.0","method":"test","params":{}}`)
		assert.NoError(t, err)
		assert.Equal(t, transport.BaseMessageTypeJSONRPCNotificationType, msg.Type)
		assert.Equal(t, "2.0", msg.JsonRpcNotification.Jsonrpc)
		assert.Equal(t, "test", msg.JsonRpcNotification.Method)
	...
	t.Run("error", func(t *testing.T) {
		msg, err := deserializeMessageV2(`{"jsonrpc":"2.0","id":1,"error":{"code":-32700,"message":"Parse error"}}`)
		assert.NoError(t, err)
		assert.Equal(t, transport.BaseMessageTypeJSONRPCErrorType, msg.Type)
		assert.Equal(t, "2.0", msg.JsonRpcError.Jsonrpc)
		assert.Equal(t, -32700, msg.JsonRpcError.Error.Code)
...
func BenchmarkDeserializeMessage(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessage(`{"jsonrpc":"2.0","id":1,"method":"test","params":{}}`)
	}
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessage(`{"jsonrpc":"2.0","method":"test","params":{}}`)
	}
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessage(`{"jsonrpc":"2.0","id":1,"error":{"code":-32700,"message":"Parse error"}}`)
	}
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessage(`{"jsonrpc":"2.0","id":1,"result":{}}`)
	}
}

func BenchmarkDeserializeMessageV2(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessageV2(`{"jsonrpc":"2.0","id":1,"method":"test","params":{}}`)
	}
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessageV2(`{"jsonrpc":"2.0","method":"test","params":{}}`)
	}
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessageV2(`{"jsonrpc":"2.0","id":1,"error":{"code":-32700,"message":"Parse error"}}`)
	}
	for i := 0; i < b.N; i++ {
		_, _ = deserializeMessageV2(`{"jsonrpc":"2.0","id":1,"result":{}}`)
	}
}
